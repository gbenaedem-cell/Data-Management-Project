...
CREATE TABLE staging.student_info (
    code_module VARCHAR(10),
    code_presentation VARCHAR(10),
    id_student INT,
    gender CHAR(1),
...
CREATE TABLE staging.student_assessment (
    id_assessment INT,
    id_student INT,
    date_submitted INT,
    is_banked SMALLINT,
...
CREATE TABLE staging.student_registration (
    code_module VARCHAR(10),
    code_presentation VARCHAR(10),
    id_student INT,
...
CREATE TABLE staging.student_vle (
    code_module VARCHAR(10),
    code_presentation VARCHAR(10),
    id_student INT,
    id_site INT,
...
CREATE TABLE dw.dim_course (
    ...
    module_presentation_length INT,
    CONSTRAINT unq_course UNIQUE (code_module, code_presentation)
);
...
CREATE TABLE dw.dim_assessment (
    assessment_key SERIAL PRIMARY KEY,
    id_assessment INT UNIQUE NOT NULL,
    course_key INT REFERENCES dw.dim_course(course_key),
    assessment_type VARCHAR(10),
    assessment_date_days INT,
    weight NUMERIC(5, 2)
);

CREATE TABLE dw.dim_vle_material (
    vle_key SERIAL PRIMARY KEY,
    id_site INT UNIQUE NOT NULL,
    course_key INT REFERENCES dw.dim_course(course_key),
    activity_type VARCHAR(50),
    week_from INT,
    week_to INT
...
-- Fact Tables
CREATE TABLE dw.fact_student_enrollment (
    enrollment_key SERIAL PRIMARY KEY,
    student_key INT REFERENCES dw.dim_student(student_key),
    course_key INT REFERENCES dw.dim_course(course_key),
...
CREATE TABLE dw.fact_student_assessment (
    submission_key SERIAL PRIMARY KEY,
    student_key INT REFERENCES dw.dim_student(student_key),
    assessment_key INT REFERENCES dw.dim_assessment(assessment_key),
    date_submitted INT,
    is_banked BOOLEAN,
    score NUMERIC(5, 2),
...
CREATE TABLE dw.fact_student_vle (
    vle_activity_key BIGSERIAL PRIMARY KEY,
    student_key INT REFERENCES dw.dim_student(student_key),
    course_key INT REFERENCES dw.dim_course(course_key),
    vle_key INT REFERENCES dw.dim_vle_material(vle_key),
